import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { apiBase } from '../types';
import '../styles/hacker.css';

const ZWaveScanPage: React.FC = () => {
  const [devices, setDevices] = useState<any[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [isScanned, setIsScanned] = useState<boolean>(false);
  const [aiAnalysis, setAiAnalysis] = useState<string>('');
  const navigate = useNavigate();

  const handleScan = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${apiBase}/api/zwave/scan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
      if (!response.ok) throw new Error("Couldn't scan devices");
      const fetchedDevices = await response.json();
      setDevices(fetchedDevices.devices);
      setAiAnalysis(fetchedDevices.devices[0].ai_result || 'No AI analysis available');
      setIsScanned(true);
    } catch (error) {
      console.error('Error scanning or analyzing devices:', error);
      setAiAnalysis('Failed to retrieve AI insights.');
    }
    setLoading(false);
  };

  useEffect(() => {
    const fetchDevices = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${apiBase}/api/zwave/get-devices`, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
        });
        if (!response.ok) throw new Error("Couldn't get devices");
        const fetchedDevices = await response.json();
        setDevices(fetchedDevices.devices);
        setAiAnalysis(fetchedDevices.devices[0].ai_result || 'No AI analysis available');
        setIsScanned(true);
      } catch (error) {
        console.error('Error fetching devices:', error);
      }
      setLoading(false);
    };

    fetchDevices();
  }, []);

  return (
    <div className="container mt-5 cyberpunk-theme">
      <style>{`
        .card {
          background: linear-gradient(to right, #4b0082, #3d0085);
          border: 4px solid #00ff91;
          border-radius: 16px;
          color: #00ff91;
          font-family: 'Courier New', monospace;
          max-width: 1300px;
          margin: 0 auto;
        }

        .btn-dark, .btn-danger, .btn-light {
          font-family: 'Courier New', monospace;
          font-size: 1.6rem;
          font-weight: bold;
          padding: 1rem;
          border-radius: 10px;
        }

        .btn-danger {
          background-color: transparent;
          border: 2px solid #00ff91;
          color: #00ff91;
        }

        .btn-light {
          background-color: rgb(26, 104, 68);
          border: 2px solid rgb(38, 56, 48);
          color: #0a0d2a;
        }

        .info-text {
          font-size: 1.3rem;
          color: #7fffd4;
          margin-bottom: 20px;
        }

        h2, p, li {
          font-size: 2rem;
        }

        .hover-jump {
          transition: transform 0.3s ease, box-shadow 0.3s ease;
          width: 100%;
          max-width: 700px;
        }

        .hover-jump:hover {
          transform: scale(1.04);
          box-shadow: 0 0 12px #00ff91;
        }

        .box-result {
          background-color: rgba(10, 15, 40, 0.95);
          border: 2px dashed #00ff91;
          color: #00ff91;
          font-size: 1.6rem;
          border-radius: 10px;
          display: flex;
          flex-direction: column;
          align-items: center;
          gap: 1rem;
          width: 100%;
        }

        .ai-panel {
          background: linear-gradient(to bottom right, #4b0082, #3d0085);
          border: 3px solid #00ff91;
          border-radius: 16px;
          padding: 20px;
          color: #00ff91;
          font-family: 'Courier New', monospace;
          height: 100%;
        }

        .ai-header {
          font-size: 2.2rem;
          color: #00ffd0;
          margin-bottom: 1rem;
        }

        .ai-info-text {
          font-size: 1.4rem;
          margin-bottom: 1.2rem;
          color: #7fffd4;
        }

        .ai-analysis-box {
          background-color: rgba(0, 0, 0, 0.3);
          border: 2px dashed #00ff91;
          padding: 1.5rem;
          border-radius: 12px;
          font-size: 1.6rem;
          line-height: 2.4rem;
          white-space: pre-wrap;
          overflow-y: auto;
          max-height: 500px;
        }

        .form-check-input:checked {
          background-color: #00ff91;
          border-color: #00ff91;
        }

        .form-control {
          background-color: #151a3d;
          color: #00ff91;
          border: 2px solid #00ff91;
          font-family: 'Courier New', monospace;
        }

        .scan-button-container {
          display: flex;
          justify-content: center;
          margin-bottom: 20px;
          width: 100%;
        }

        .scan-button {
          width: 90%;
        }

        /* AI and Device List Panel Styling */
        .left-panel {
          flex: 1;
          padding-right: 20px;
        }

        .right-panel {
          flex: 2;
        }

        /* Flex container for side-by-side layout */
        .row-flex {
          display: flex;
          flex-direction: row;
          justify-content: space-between;
        }
      `}</style>

      <div className="row justify-content-center">
        <div className="col-lg-12">
          <div className="card shadow-lg p-5 text-center">
            <h2 className="mb-3">Z-Wave Device Scanner</h2>
            <p className="info-text">
              This tool scans your local network and identifies connected Z-Wave devices. Click scan to view results and AI-powered insights.
            </p>

            <div className="scan-button-container">
              <button
                className="btn btn-light scan-button"
                onClick={handleScan}
                disabled={loading}
              >
                {loading ? 'Scanning...' : 'Scan Z-Wave Devices'}
              </button>
            </div>

            {/* AI and Device List Panel */}
            <div className="row-flex">
              {/* Left: AI Analysis Panel */}
              <div className="left-panel">
                <div className="ai-panel mb-4">
                  <h3 className="ai-header">Security Review</h3>
                  <p className="ai-info-text">Review findings based on scanned data.</p>
                  <div className="ai-analysis-box">
                    {aiAnalysis ? aiAnalysis : 'Scan devices to see AI insights.'}
                  </div>
                </div>
              </div>

              {/* Right: Scanned Devices */}
              <div className="right-panel">
                <div className="box-result p-4 mb-3">
                  {loading ? (
                    <div>Scanning in progress...</div>
                  ) : (
                    <div>
                      {isScanned ? (
                        devices.length ? (
                          devices.map((device, index) => (
                            <div className="hover-jump mb-3" key={index}>
                              <div className="card p-3 text-center" style={{ borderRadius: '10px' }}>
                                <h5>{device.name}</h5>
                                <p>{device.mac}</p>
                              </div>
                            </div>
                          ))
                        ) : (
                          <p>No devices found</p>
                        )
                      ) : (
                        <p>No scan results yet.</p>
                      )}
                    </div>
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ZWaveScanPage;
